...
foreach ($schedules as $schedule) {
    ...
    // Check if there is no user log entry for the current schedule
    $userLogExists = false;
    ...
    // If there is a leave entry, insert a leave record
    if ($leaveExists) {
    ...
    // If no user log entry exists, insert an absent record
    elseif (!$userLogExists) {
        $currentDate = date('Y-m-d'); // Get the current date
        ...
        echo '<script>alert("User already has a user log entry for the current schedule");</script>';
        echo '<script>setTimeout(function() { window.location = "userlog.php"; }, 100);</script>';
    }
}
}
